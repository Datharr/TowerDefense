...
def check_input_exit():  #echap = quitters
...
def arrow_traj(monster_list,building_list, far, far2, projectile_list, a, data, arrow, effect_list): #codage des flèches
    ...
    elif building_list[a].type == 2:
        for c in range (len(monster_list)):
            if c in building_list[a].targetable:
                monster_list[c].pv -= monster_list[c].pvmax/100*0.75
                if monster_list[c].pv <= 0:
                    monster_list.pop(c)
...
def build(monster_list,logo1_rect,logo3_rect,logo4_rect,logo5_rect,building_list,projectile_list,State,data):   #cliquer sur le bouton en bas a gauche met en mode "construction"
    ...
    if data["money"] >= 100:
        if mouse_p[0] == True:
            if logo1_rect.collidepoint(mouse_pos):
                ...
                State = 3
            elif logo5_rect.collidepoint(mouse_pos) and data["money"] >= 150:
                State = 4    
...
def cursor(State,building_list,data,effect_list): #fonction pour construire un batiment une fois en mode "construction"
    if State == 1 :
        ...
        mouse_p = pygame.mouse.get_pressed()
        if mouse_p[0] == True:
            building_list.append(Tower(mouse_pos[0],mouse_pos[1],0,0,0,0,2,270))
            projectile_list.append({"x":mouse_pos[0],"y":mouse_pos[1],"reset_x":mouse_pos[0],"reset_y":mouse_pos[1],"cd":0,"cdmax":10})
            data["money"] -= 300
            State = 0
            effect_list = load_effect(effect_list, mouse_pos[0], mouse_pos[1], "animation/build_effect/", 3, 32, 1, 1)
        ...
        win.blit(barrack1,(mouse_pos))
        mouse_p = pygame.mouse.get_pressed()
        if mouse_p[0] == True:
            building_list.append(Tower(mouse_pos[0],mouse_pos[1],0,0,0,0,3,270))
            projectile_list.append({"x":mouse_pos[0],"y":mouse_pos[1],"reset_x":mouse_pos[0],"reset_y":mouse_pos[1],"cd":0,"cdmax":10})
            data["money"] -= 150
            State = 0
            effect_list = load_effect(effect_list, mouse_pos[0], mouse_pos[1], "animation/build_effect/", 3, 32, 1, 1)
...
def shop(building_list,data,a,mouse_pos,mouse_p,circle_shop,item1,item2,item3,item20):
    ...
    if building_list[a].type == 2:
        if building_list[a].addon.check_mouse(mouse_pos):
            ...
            if building_list[a].upgrade["Range"] == None:
                win.blit(item20, (building_list[a].x-15,building_list[a].y+90))
                rect_topleft = pygame.Rect(building_list[a].x-15,  building_list[a].y+90, 24, 24)
                if mouse_p[0] == True and data["money"] >= 100:
                    if rect_topleft.collidepoint(mouse_pos):
                        data["money"] -= 100 
                        building_list[a].structure = 2
                        building_list[a].upgrade["Range"] = 1
                        building_list[a].upgrade_range(320)
...
def hud_load_item():
    ...
    item20 = pygame.image.load("images/items/Item__20.png").convert_alpha()
    item20 = pygame.transform.scale(item20, (24, 24))

    return item1,item2,item3,item20
...
def hud_load_tower():
    ...
    barrack1 = pygame.image.load("images/barrack1.png").convert_alpha()
    barrack1 = pygame.transform.scale(barrack1, (80, 100))

    return tower1,tower2,tower3,tower4,barrack1
...
def hud_load_wizard_tower():
    ...
    wizard4 = pygame.image.load("images/wizard4.png").convert_alpha()
    wizard4 = pygame.transform.scale(wizard4, (80, 100))

    return wizard1,wizard2,wizard3,wizard4
...
def hud_load_hud():
    ...
    build_logo = pygame.image.load("images/logotower.jpg").convert()
    ...
    logo_range = pygame.image.load("images/logorange.jpg").convert()
    logo_range = pygame.transform.scale(logo_range, (118, 104))
    barracks_logo = pygame.image.load("images/logobarrack.jpg").convert()
    barracks_logo = pygame.transform.scale(barracks_logo, (118, 104))
    spd_logo = pygame.image.load("images/x1.jpg").convert()
    spd_logo = pygame.transform.scale(spd_logo, (118, 104))
    rect_blue = pygame.image.load("images/rect_blue.png").convert_alpha()
    rect_blue = pygame.transform.scale(rect_blue, (40, 40))
    health_bar = pygame.image.load("hud/health_bar/health_bar.png").convert_alpha()
    health_bar  = pygame.transform.scale(health_bar, (245, 85))
    health_heart = pygame.image.load("hud/health_bar/health_bar_decoration.png").convert_alpha()
    health_heart   = pygame.transform.scale(health_heart, (320, 85))

    return circle_shop,lvl1,build_logo,wizard_logo,spd_logo,lava_logo,logo_range,barracks_logo,rect_blue,health_bar,health_heart

def hud_load_everything():

    circle_shop,lvl1,build_logo,wizard_logo,spd_logo,lava_logo,logo_range,barracks_logo,rect_blue,health_bar,health_heart = hud_load_hud()
    wizard1,wizard2,wizard3,wizard4 = hud_load_wizard_tower()
    tower1,tower2,tower3,tower4,barrack1 = hud_load_tower()        
    barre_list = hud_load_barre()
    item1,item2,item3,item20 = hud_load_item()

    return circle_shop,lvl1,build_logo,wizard_logo,spd_logo,lava_logo,logo_range,barracks_logo,wizard1,wizard2,wizard3,wizard4,tower1,tower2,tower3,tower4,barrack1,barre_list,item1,item2,item3,item20,rect_blue,health_bar,health_heart

def monster_pass(monster_list,data,health_data):
    ...
    return monster_list,data,health_data
    
def show_range(data,logo6_rect):
    mouse_pos = pygame.mouse.get_pos()
    mouse_p = pygame.mouse.get_pressed()
    if logo6_rect.collidepoint(mouse_pos):
        if mouse_p[0] == True:
            if data["range"] == True:
                data["range"]=False
            elif data["range"] == False:
                data["range"]=True
    return data

def hero_idle(hero):

    if hero["statut"] == "RunningLeft":
        hero_move = pygame.image.load("heroes/ronin/left/run/run"+ str(hero["movement"])+".png").convert_alpha()
        if hero["movement"] >= hero["runmax"]:
            hero["movement"] = 1
    elif hero["statut"] == "RunningRight":
        hero_move = pygame.image.load("heroes/ronin/right/run/run"+ str(hero["movement"])+".png").convert_alpha()
        if hero["movement"] >= hero["runmax"]:
            hero["movement"] = 1
    elif hero["statut"] == "Idle":
        hero_move = pygame.image.load("heroes/ronin/right/idle/idle"+ str(hero["movement"])+".png").convert_alpha()
        if hero["movement"] >= hero["idlemax"]:
            hero["movement"] = 1
    
    hero_move = pygame.transform.scale(hero_move, (74, 104))

    hero["movement"] += 1
    return hero,hero_move

def move_hero(hero):
    mouse_p = pygame.mouse.get_pressed()
    if mouse_p[2] == True:
        mouse_pos = pygame.mouse.get_pos()
        hero["location_x"] = mouse_pos[0]-45
        hero["location_y"] = mouse_pos[1]-50
        hero["statut"] = "RunningRight"
         
    return hero

def hero_traj(hero,location):
    if hero["x"] >= hero["location_x"]-10 and hero["x"] <= hero["location_x"]+10:
        hero["x"] = hero["location_x"]  
    elif hero["x"] < hero["location_x"]:
        hero["x"] += 5
        hero["statut"] = "RunningRight"
    else: 
        hero["x"] -= 5
        hero["statut"] = "RunningLeft"

    if hero["y"] >= hero["location_y"]-10 and hero["y"] <= hero["location_y"]+10:
        hero["y"] = hero["location_y"]  
    elif hero["y"] < hero["location_y"]:
        hero["y"] += 5
    else: 
        hero["y"] -= 5

    
    if hero["x"] == hero["location_x"] and hero["y"] == hero["location_y"]  :
        hero["statut"] = "Idle"
...
def sound():
        pygame.mixer.music.load("sound/son.mp3")
...
if __name__ == "__main__":  #programme main 
    ...
    clock = pygame.time.Clock()           #gestion fps    #A 
    win = pygame.display.set_mode((1920, 1080), pygame.FULLSCREEN)   #A  
    State = 0
    wave_state = 0
    b = 1
    hero = {"movement":1,"idlemax":8,"runmax":8,"x":-50,"y":439,"statut":"RunningLeft","location_x":30,"location_y":439}  #A
    data = {"pv":100,"money":1000,"range":False} #A
    sound()
    
    monster_list = [] #A

    building_list = [
        Tower(1412,100,0,0,0,0,0,500) #A 
    ]

    projectile_list = [{"x":1412,"y":100,"reset_x":1412,"reset_y":100,"cd":0,"cdmax":10} #A

    ]

    wave_list = [[0,0,0,0,0,0],[5,2,10,2,2,2,3,5],[1,10,0,1,1,0],[2,15,2,1,2,0]]
    actual_wave = wave_list[0]
    wave_number = 0
    effect_list = []
    actual_spd = 20
    load_effect(effect_list, -13, 100, "animation/coin/", 2, 12, 1, 4)
    ...
    logo5_rect = pygame.draw.rect(win, (0, 200, 0), (341,  978, 118, 104))
    logo6_rect = pygame.draw.rect(win, (0, 200, 0), (113,  876, 118, 104))
    
    
    
    for a in range (len(monster_list)):                          #A  -- 
        monster_list[a].path = random.randint(0, 1)
        if monster_list[a].path == 0:
            monster_list[a].y = random.randint(223, 300)
    ...
    circle_shop,lvl1,build_logo,wizard_logo,spd_logo,lava_logo,logo_range,barracks_logo,wizard1,wizard2,wizard3,wizard4,tower1,tower2,tower3,tower4,barrack1,barre_list,item1,item2,item3,item20,rect_blue,health_bar,health_heart = hud_load_everything()
    arrow = rotate_arrow("images/arrow.png",0)

    health_data = [315,35,10,35]
    ...
    while True:                                                       #boucle de jeu
        ...
        win.blit(barracks_logo, (341,978))
        win.blit(logo_range, (113,876))
        win.blit(spd_logo, (-5,876))
        win.blit(health_heart,(10,10))
        win.blit(health_bar,(80,10))
        health = pygame.draw.rect(win, (40, 40, 40), (health_data[0],  health_data[1], health_data[2], health_data[3]))
        ...
        building_list,projectile_list,State = build(monster_list,logo1_rect,logo3_rect,logo4_rect,logo5_rect,building_list,projectile_list,State,data)
        monster_list = movement_goblin(monster_list)
        actual_spd,spd_logo = spd_button(actual_spd,spd_logo)
        display_text(win,str(data["money"]),110, 135, 50,0,0,255)                      #affichage de l'argent avec (fenêtre,texte,x,y,taille,(couleur RGB))
        data = show_range(data,logo6_rect)
        hero,hero_move = hero_idle(hero)
        hero = move_hero(hero)

        if hero["statut"] != "Idle" :
            hero = hero_traj(hero,100)
        win.blit(hero_move, (hero["x"],hero["y"]))
        ...
        for a in range (len(building_list)):
            if data["range"] == True:
                pygame.draw.rect(win, [255, 0, 0], [building_list[a].hitbox.pos_x,building_list[a].hitbox.pos_y,building_list[a].hitbox.size_x,building_list[a].hitbox.size_y], 4) #enlever le # permet de voir les hitbox

            building_list,data = shop(building_list,data,a,mouse_pos,mouse_p,circle_shop,item1,item2,item3,item20) 
            if building_list[a].type == 0:
                if building_list[a].structure == 1:
                    win.blit(tower2, (building_list[a].x, building_list[a].y))
                elif building_list[a].structure == 3:
                    win.blit(tower3, (building_list[a].x, building_list[a].y))
                elif building_list[a].structure == 4:
            ...
            if building_list[a].type == 2:
                ...
                elif building_list[a].structure == 2:   
                    win.blit(wizard4, (building_list[a].x, building_list[a].y)) 
            if building_list[a].type == 3:
                win.blit(barrack1, (building_list[a].x, building_list[a].y))
        ...
        monster_list,data,health_data = monster_pass(monster_list,data,health_data)
